...
public class WireMockStubManager {
	...
	public static void setupStubs() {
		...
		WireMock.stubFor(WireMock.post("/api/now/table/incident")
				...
				.willReturn(WireMock.aResponse().withStatus(403).withBody("{\"error\":\"Forbidden: Access Denied\"}")));

		// 🔹 4. 400 - Bad Request (Invalid Input Data)
		WireMock.stubFor(WireMock.post("/api/now/table/incident")
				.withRequestBody(matchingJsonPath("$.short_description", absent())).willReturn(WireMock.aResponse()
						.withStatus(400).withBody("{\"error\":\"Bad Request: 'short_description' is required\"}")));

		// 🔹 5. 500 - Internal Server Error (Unexpected System Issue)
		WireMock.stubFor(WireMock.post("/api/now/table/incident")
				.withRequestBody(matchingJsonPath("$.causeError", equalTo("true"))).willReturn(WireMock.aResponse()
						.withStatus(500).withBody("{\"error\":\"Internal Server Error: Unexpected issue occurred\"}")));
